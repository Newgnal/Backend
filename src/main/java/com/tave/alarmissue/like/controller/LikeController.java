...
public class LikeController {
    ...
    public ResponseEntity<LikeResponseDto> commentLike(@AuthenticationPrincipal PrincipalUserDetails principal,@PathVariable Long postId,@PathVariable Long commentId) {

        Long userId = principal.getUserId();

        LikeResponseDto responseDto = likeService.commentLike(userId,postId,commentId);
        return ResponseEntity.status(HttpStatus.CREATED).body(responseDto);
